...
## Lifecycle: cancel, crash, restart, recover

- [2026-08-05 · source: 24-execution-profiles-lifecycle-axis] Before you report `confirmed` on round one, walk the lifecycle axis explicitly: what happens if this is cancelled mid-flight, if the process crashes here, if it restarts and recovers, and if the resource this depends on no longer exists. Implementations that satisfy the specified behavior and its direct adversarial inputs still fail review on this axis: released-but-still-running leases, recovery paths that skip re-acquisition, stale metadata surviving a crash, and chained symlinks that escape a check that only looked one level deep. Auditing it before handback costs one pass; missing it costs a full round.
