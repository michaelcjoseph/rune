...
const TECH_LEAD_INSTRUCTION = [
  ...
  '```tech-breakdown',
  '{"tasks": [{"id": "<stable-slug>", "text": "<what this task delivers>", "phase": "Phase 1 - Core", "testStrategy": "code-tests-required|docs-or-config-only|tests-as-deliverable", "validationPolicy": "required|reviewed-no-validation", "designerNeeded": false, "securityNeeded": false, "roles": ["qa", "coder", "reviewer", "tech-lead"]}], "perProjectExemplars": {"qa": "<markdown exemplar for this project, if useful>"}}',
  '```',
  '```tech-spec',
  ...
  'Set securityNeeded true ONLY when the implementation diff requires the',
  'security product-team role as an explicit closeout gate.',
...
function parseSizedTask(raw: unknown, index: number): SizedTask {
  ...
  return {
    ...
    securityNeeded: t['securityNeeded'] === true,
...
const PM_REVIEW_INSTRUCTION = [
  ...
  '```pm-review',
  '{"match": false, "mismatches": ["<each concrete gap>"], "repairSummary": "<what you changed>", "repairedTasks": [{"id": "<stable-slug>", "text": "<deliverable>", "phase": "Phase 1 - Core", "testStrategy": "code-tests-required|docs-or-config-only|tests-as-deliverable|manual-live-gate", "validationPolicy": "required|reviewed-no-validation", "designerNeeded": false, "securityNeeded": false, "roles": ["qa", "coder", "reviewer"]}]}',
  '```',
...
const CRITIQUE_INSTRUCTION = [
  ...
  'code-tests-required|docs-or-config-only|tests-as-deliverable|manual-live-gate, designerNeeded boolean, securityNeeded boolean, roles array).',
  ...
  '```critique-tasks',
  '{"tasks": [{"id": "<stable-slug>", "text": "<deliverable>", "phase": "Phase 1 - Core", "testStrategy": "code-tests-required", "validationPolicy": "required", "designerNeeded": false, "securityNeeded": false, "roles": ["qa", "coder", "reviewer"]}]}',
  '```',
  '```critique-spec',
