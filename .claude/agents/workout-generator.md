...
- **Resolve location & focus.** If both in args, use them. If one missing, infer from recent_workouts (variety rotation), recent_whoop (low recovery → mobility/endurance), and goals. Default location: most-recent workout's location, else `home`.
