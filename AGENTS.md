...
All AI operations use Codex CLI (Max subscription, no API key needed). Custom agents in `.Codex/agents/` handle structured KB operations (wiki-compiler, kb-query, wiki-linter).
...
├── ai/Codex.ts             # All Codex CLI spawning: askClaude, runAgent, summarizeSession; exports setBus(bus) — called from index.ts so runAgent() can emit BusAgentEvent frames (type-only NotificationBus import avoids circular dep); runAgent() appends {agent, startedAt, durationMs, status} to logs/agent-runs.jsonl after each invocation; exports CLAUDE_BIN (resolved binary path), registerActiveProcess/unregisterActiveProcess (for external spawners like work-runner)
├── bot/
...
│       ├── ask.ts           # /ask — one-shot freeform Codex question (legacy escape hatch; no longer a resolver route)
...
│       ├── cancel.ts        # /cancel [opId-prefix] — SIGTERM an in-flight Codex op (most recent for user, or by id prefix)
...
│   ├── in-flight.ts         # In-flight Codex-op registry: registerOp/unregisterOp/cancelOp/cancelMostRecentForUser/listOps; 5s heartbeat ticker emits op-event:progress; setInFlightBus(bus) wires bus emission; cancelled flag overrides exit status to 'cancelled'
...
│   ├── state-snapshot.ts    # StateSnapshot type + getStateSnapshot(): reads logs/agent-runs.jsonl, scheduler-state.json, active session/review, ingestion queue, playbook/proposal counts, project summaries, active+recent mutations, in-flight Codex ops; used by GET /api/state
...
│   ├── work-runner.ts       # workRunApplier: MutationApplier for 'work-run' kind; spawns Codex CLI with spec.md+tasks.md+/work --auto in project dir; streams stdout/stderr as MutationEvents; enforces per-project and global concurrency caps
...
│   └── index.ts             # Standalone stdio entry point for Codex
├── integrations/
...
3. Each post-agent runs in parallel. Failures and missing-agent errors are surfaced in the TG summary (not silent) — see `AGENT_NOT_FOUND_PREFIX` in `src/ai/Codex.ts`.
...
- Codex CLI spawning is centralized in `src/ai/Codex.ts` — never spawn `Codex` directly elsewhere
...
- Autonomous codebase operations go through the mutation pipeline (`src/transport/mutations.ts`) — register a `MutationApplier`, call `createMutation()`, never spawn Codex CLI for project work directly. `CLAUDE_BIN`, `registerActiveProcess`, and `unregisterActiveProcess` from `src/ai/Codex.ts` keep binary resolution and shutdown tracking centralized for external spawners.
...
| Agent | File | Purpose |
|---|---|---|
| wiki-compiler | `.Codex/agents/wiki-compiler.md` | Ingest raw sources → create/update wiki pages |
| kb-query | `.Codex/agents/kb-query.md` | Search wiki + vault → synthesized answer |
| wiki-linter | `.Codex/agents/wiki-linter.md` | Health-check wiki for issues |
| morning-prep | `.Codex/agents/morning-prep.md` | Gather vault data → structured morning journal section |
| session-summarizer | `.Codex/agents/session-summarizer.md` | Rich session summaries with vault context |
| release-notes | `.Codex/agents/release-notes.md` | Generate changelog from git history |
| content-triager | `.Codex/agents/content-triager.md` | Classify URLs/text → kb-ingest, readwise, journal, or skip |
| photo-classifier | `.Codex/agents/photo-classifier.md` | Classify photos → book, receipt, whiteboard, etc. with routing |
| system-scanner | `.Codex/agents/system-scanner.md` | Review prep: summarize current state of health/study/psychology/etc. |
| project-updater | `.Codex/agents/project-updater.md` | Post-review: apply approved updates to projects/*.md |
| playbook-proposer | `.Codex/agents/playbook-proposer.md` | Nightly: draft playbook entries from `#playbook`-tagged journals |
| playbook-updater | `.Codex/agents/playbook-updater.md` | Post-review: append approved drafts to pages/playbook.md |
| proposal-updater | `.Codex/agents/proposal-updater.md` | Post-review: action approved Ask-Twice proposals — creates new agent files under `.Codex/agents/` and/or registers cron frontmatter on existing agents; marks actioned entries in `logs/proposal-queue.json` |
| worldview-updater | `.Codex/agents/worldview-updater.md` | Post-review: apply approved diffs to world-view/*.md with changelog entry |
| psychology-updater | `.Codex/agents/psychology-updater.md` | Post-review: apply scoped updates to pages/psychology.md |
| json-updater | `.Codex/agents/json-updater.md` | Post-review / nightly: apply updates to JSON data stores |
| daily-content-updater | `.Codex/agents/daily-content-updater.md` | Nightly daily-tags: apply updates to markdown content stores (`health/nutrition.md`, `projects/ideas.md`, `writing/topics.md`) |
| intent-scan | `.Codex/agents/intent-scan.md` | Saturday 3pm cron: runs `npm run intent-scan` to process intent-log and write skill proposals |
| workout-generator | `.Codex/agents/workout-generator.md` | Generates a one-shot daily workout (warmup → main → cooldown) tailored to goals, equipment, recent training load, Whoop recovery, and exercise preferences |
| lenny-sync | `.Codex/agents/lenny-sync.md` | Pull new Lenny posts/podcasts via MCP into library/lenny/, update logs/lenny-sync-state.json |

### Vault-resident agents (personal content, loaded from `$VAULT_DIR/.Codex/agents/`)

`loadAgentDef` in `src/ai/Codex.ts` checks Jarvis's agents dir first, then falls back to the vault. The following agents live only in the vault because their instructions encode personal specifics (family names, employer, project codenames) that don't belong in a public repo:
...
| Agent | File | Purpose |
|---|---|---|
| test-specialist | `.Codex/agents/test-specialist.md` | Bootstrap vitest, write tests, run them |
| code-reviewer | `.Codex/agents/code-reviewer.md` | Review for bugs, security, convention violations |
| security-auditor | `.Codex/agents/security-auditor.md` | Audit for secrets, PII exposure, vault leaks, server security |
| architecture-reviewer | `.Codex/agents/architecture-reviewer.md` | Review for system-level architectural issues |
| code-simplifier | `.Codex/agents/code-simplifier.md` | Check for dead code, over-abstraction, duplication |
| docs-sync | `.Codex/agents/docs-sync.md` | Update AGENTS.md and docs after structural changes |
| json-updater | `.Codex/agents/json-updater.md` | Update JSON config files programmatically |

## MCP Server

The knowledge base is exposed as an MCP server so any Codex session can query, search, ingest, and lint the KB.

**Config**: `.Codex/settings.json` registers `jarvis-kb` MCP server.
