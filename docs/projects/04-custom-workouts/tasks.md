...
- [ ] Create `health/equipment.md` in vault with `## Home` and `## Gym` sections. Seed Home from existing `plan.md` equipment lines (adjustable kettlebells, barbell + plates). Gym section can start empty or with a placeholder — hand-fill after next gym visit.
