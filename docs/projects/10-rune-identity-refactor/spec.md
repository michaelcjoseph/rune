...
compiler (`rune/bin/compile-instructions`) with an explicit IR, pure-function
`claude`/`agents` renderers, a YAML manifest, a `$RUNE_HOME` wrapper, a named-token
inventory verifier, CI drift checks, and a per-repo migration playbook — then migrate
rune and four consumer repos (pkms, aura, assay, relay), each via its own project.
...
2. **Move** the orchestrator-identity sections out of `pkms/CLAUDE.md` (where they bleed
   vault and orchestrator concerns together) into `rune/CLAUDE.md`, leaving a one-line
   pointer behind.
...
- **No divergence will ever exist between `CLAUDE.md` and `AGENTS.md`.** Confirmed
  ...
  `$RUNE_HOME` / CI-cascade layer existed only to manage divergence that the design
...
- **The two real problems are small.** The acute pain is (a) the files have drifted
  (rune `AGENTS.md` frozen at 2026-05-19 while `CLAUDE.md` advanced through projects
...
`$RUNE_HOME` wrapper, named-token inventory verifier, CI drift-check cascade, the
...
1. **Drift.** `rune/CLAUDE.md` (408 lines, last edited 2026-06-01, carries the project
   08–09 cockpit / planning-session / mutation-pipeline updates) vs `rune/AGENTS.md`
...
2. **Misplaced identity.** `pkms/CLAUDE.md` carries `## Rune` (agent split, KB
   ...
   Rune *orchestrator* behaves, not how the *vault* is structured. They belong in rune.
...
| ----- | ----------------------------------- | --------------------------------------------------- |
| rune | both files, drifted                | `git rm AGENTS.md` → symlink to `CLAUDE.md`          |
...
Cut the orchestrator sections from `pkms/CLAUDE.md`, paste them into `rune/CLAUDE.md`,
leave a pointer in pkms. One commit per repo. The git diff is the proof nothing was lost —
read it.

**Moving out of `pkms/CLAUDE.md` → into `rune/CLAUDE.md`:**

- The entire `## Rune` section — automation ownership (morning prep, nightly job),
  the agent split (generic tooling agents in rune vs personal-specifics agents in
...
> and the review write-up pipeline — is documented in `rune/CLAUDE.md`.
...
- **Does Codex read through the symlink?** Almost certainly yes — it opens `AGENTS.md`
  ...
  confirm before committing the rune symlink (test-plan §1). **Fallback if it does
...
- **git symlink support.** macOS/Linux only (rune runs on both); `core.symlinks`
...
- **Content loss in the move.** Mitigated by reading the two-repo git diff: the moved
  sections must appear verbatim in `rune/CLAUDE.md` and be absent from `pkms/CLAUDE.md`,
...
- [ ] Append the `## Rune` and `### How Reviews Work` sections to `rune/CLAUDE.md`
      (placed coherently within its existing structure).
- [ ] Remove those sections from `pkms/CLAUDE.md`; insert the pointer.
- [ ] Read the diff in both repos; confirm the moved content is present in rune, absent
...
- [ ] **rune:** `git rm AGENTS.md`; `ln -s CLAUDE.md AGENTS.md`; `git add`.
- [ ] **Verify Codex loads through the symlink** in rune before proceeding (see Risks).
...
| ------ | ------ | ------------ |
| `AGENTS.md` is a symlink to `CLAUDE.md` | rune + pkms (core); assay + aura (best-effort) | `test -L AGENTS.md && readlink AGENTS.md` = `CLAUDE.md` |
| `CLAUDE.md` ≡ `AGENTS.md` bytes | identical in every touched repo | `diff CLAUDE.md AGENTS.md` exits 0 |
| Codex loads through the symlink | confirmed | manual Codex session in rune reads orchestrator identity |
| Orchestrator sections moved, not lost | 100% present in rune, absent in pkms | git diff review across both repos |
...
- **Symlink committed but Codex can't follow it** → apply the `cp` + `diff` fallback
  (Risks); revert the rune symlink, regenerate `AGENTS.md` as a copy, add the diff check.
