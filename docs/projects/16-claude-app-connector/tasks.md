...
- [ ] **remote-tunnel-exposure** (docs/config only) — Stand up a named tunnel exposing only the daemon `/mcp` route at a stable HTTPS hostname, with no inbound ports on the host. Document the tunnel config, secret handling, and recovery/runbook. (Docs/config half DELIVERED, **Funnel-first** per 2026-06-10 decision: [tunnel-runbook.md](tunnel-runbook.md) — Tailscale Funnel path mounts, env settings incl. MCP_ISSUER_URL + JARVIS_ALLOWED_HOSTS, verification curls, recovery table, Cloudflare Tunnel appendix as fallback. The live standing-up needs the operator: tailnet admin toggles + the three funnel mount commands — see the parked run.)
