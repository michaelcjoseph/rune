...
- [ ] **argv-command-executor** — Make runValidationCommandArgv the single executor for profile commands, supporting relative contained cwd, scrubbed environment overlays, timeout and process-group reaping, bounded output capture, tool-version capture, and structured start/terminal/failure logging. Profile paths must never use whitespace splitting. Tests cover argv fidelity, cwd and environment containment, timeout reaping, output bounds, and failure logging. _(security review)_
  - Test strategy: `code-tests-required`
  - Roles: qa, coder, security, reviewer
  - Pinned executor contract (tech-lead, run `9be9ac7c`; do not re-derive):
    - **Signature unchanged.** Keep `runValidationCommandArgv(argv, cwd, timeoutMs, diagnosticDir?, options)`. It does not accept a `CommandSpec`. The profile layer maps `CommandSpec` to argv plus options at the call site.
    - **New `ValidationCommandArgvOptions` fields:** `commandId: string` (required for profile commands), `containedCwd?: string`, `envOverlay?: Record<string, string>`, `toolVersions?: Record<string, string>`.
    - **`containedCwd` is relative-only,** resolved against the `cwd` argument and realpath-checked to stay inside it. Absolute paths, parent traversal, and symlink escapes fail closed before any spawn, returning `{ exitCode: null, timedOut: false, failureClass: 'cwd-escape' }`. This widens the `failureClass` union in `src/jobs/work-run-gate-runtime.ts` from `'profile-unavailable'` to `'profile-unavailable' | 'cwd-escape'`.
    - **Environment:** the child receives the scrubbed validation base environment plus `envOverlay`, nothing else. Ambient parent variables must not reach it.
    - **Lifecycle evidence on `ValidationCommandResult`:** `startedAt` and `finishedAt` (ISO 8601), `durationMs`, and `toolVersions` echoed from options.
    - **Log sink is the module's `createLogger` logger.** `info('validation-command-start', { commandId, argv, timeoutMs, profile, cwd })` and `info('validation-command-finish', { commandId, exitCode, timedOut, durationMs, toolVersions })`. Spawn failures log via `error` at the catch point with `commandId` and the original error. All paths scrubbed before they reach a record.
    - **Tool versions are supplied by the caller,** never probed by the executor.
    - **argv entries pass through literally.** No whitespace splitting, no shell.
...
- [ ] **assay-profile-implementation** — Implement Assay's profile with Python 3.12, uv provisioning, and required argv checks for uv sync --all-groups, pytest, Ruff, uv lock --check, and assay --help. External-repository changes are authored on a reviewable branch in the Assay repository and enumerated for the migration runbook. Add profile and tier-selection tests.
  - Test strategy: `code-tests-required`
  - Roles: qa, coder, reviewer
- [ ] **python-provisioning-bug-regression-closeout** — Read the complete loop-filed “Orchestrated Python worktrees have no declared or verified dependency-provisioning contract” entry in [`../bugs.md`](../bugs.md) as the authoritative acceptance contract. Map every acceptance criterion to implementation and automated regression evidence; add or repair code and tests for any gap rather than accepting configuration or agent narration as proof. The automated proof must cover the validated Python/uv/dependency-group/provisioning-source declaration, all three structured pre-dispatch failure classes, auditable source selection, `uv.lock` verification against `pyproject.toml`, the full Assay command sequence, missing-uv rejection before QA/coder dispatch, successful provisioned-fixture dispatch, and secret/host-path scrubbing. Run focused suites, `npm run build`, and the full test suite. Move the bug from `Loop-filed` to `Done` only when every criterion is evidenced and green; otherwise leave it loop-filed and add the missing corrective work here.
  - Test strategy: `code-tests-required`
  - Roles: qa, coder, reviewer
