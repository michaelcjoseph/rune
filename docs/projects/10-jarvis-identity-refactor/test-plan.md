...
Error handling and verification checklist for the canonical-source compiler and five-repo migration.
...
- [ ] 🔴 `compile-instructions --check` exits 0 in all three repos (aura, assay, relay).
- [ ] 🔴 Generated `aura/CLAUDE.md`, `assay/CLAUDE.md`, and `relay/CLAUDE.md` begin with the autogenerated header.
- [ ] 🟡 Reviewer sign-off recorded on diff of generated `CLAUDE.md` vs pre-migration snapshot for aura and assay. Allowed deltas: structural reorganization, duplication removal. Disallowed: silent loss of any named behavior, agent, command, route, or rule.
- [ ] 🟡 Reviewer sign-off recorded on relay's starter instructions (fresh scaffold; no snapshot diff applies). Confirm the starter set covers repo overview, conventions, and key commands.

### Post-merge monitors

- [ ] 🟡 Next 3 `/work` runs targeting aura complete without errors related to missing instructions.
- [ ] 🟡 Next 3 `/work` runs targeting relay complete without errors related to missing instructions.

### Integration verification

> A `/work` session targeting aura, assay, or relay reads the regenerated `CLAUDE.md` and applies its instructions without errors.
