...
Start Phase 10 with the **active-harm probe test**: assert whether a working orchestrated run
(no `output`/`activity` for the quiet window) is currently cancelled or nudged by the
quiet→cancel / quiet-nudge backstop. Write it red first to pin current behavior, so the
streaming fix (sink-pump + heartbeat-advance) is regression-guarded either way. This is the
first unchecked task in `tasks.md` Phase 10; the rest of the phase (sink-pump, heartbeat,
codex/claude stream, role-transition, provider-attribution, transcript substrate, auto-merge)
depends on the data path it establishes.
