...
- [ ] Thread `baseSha` into `createWorktree` (`git worktree add -b <branch> <path> <baseSha>`); capture the in-memory `tasks.md` baseline at spawn.
...
- [ ] Flush transcript + atomically (temp-then-rename) write `summary.json` before the terminal event.
...
- [ ] Export `bundle.git`, `diffstat.txt`, `status.txt`, `diff.patch`, staged diff, and (non-clean) `untracked.tar` to `logs/work-runs/<id>/` before the terminal event.
- [ ] Always destroy the deterministic worktree in `finally` after export.
...
- [ ] Add the work-run projection to `buildCockpitView` from the new store (id, last-N output, elapsed, outcome, reason, transcript URL).
