...
- [ ] **realtime-run-panel-ui** — Run panel inside the deep view: tasks updating in realtime (even when edits live in a separate worktree), which agents are working the run (PM/tech-lead/coder/reviewer/…), elapsed + live output + worktree path + classified outcome on completion, and the most-recent run's transcript readable without special steps. Consumes the Phase 2 subscription module + `/live` snapshot. _(designer review)_
