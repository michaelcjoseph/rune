...
`src/roles/loader.ts` generalizes the writer-role loader pattern to the seven product-team roles (pm, tech-lead, qa, coder, reviewer, designer, adjudicator). Each role has a SOUL charter and a `memory.md` under `agents/<role>/`. The same two-channel authority boundary applies: SOUL.md → system-prompt, memory.md → low-authority reference fence.
