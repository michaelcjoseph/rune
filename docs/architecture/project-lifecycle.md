...
| D3 | **Hard merge gate** | `runGate` (under per-product base-branch lock) | first-failure-wins: validation-present → no concurrent run → **clean dry-merge** → **zero tasks remaining** → clean tree → **validation green**. **Green-suite gate #2.** | `{ok:true}` | `jobs/work-run-gate-runtime.ts` `runGate`; `work-run-gate.ts` `evaluateGate`; `work-run-merge-lock.ts` `withBaseBranchLock` |
