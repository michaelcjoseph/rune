...
Once canonical capture succeeds, `TaskEvidence` retains optional `taskBaseTree`, `currentReviewTree`, and `fullTaskReviewHash` metadata on every later return path—including malformed verdicts, gate rejections, cancellations, and executor failures. `TaskRunRecord` carries the same optional fields for historical JSONL compatibility. Raw diffs stay transient; transcript/task-record/diagnostic surfaces contain only bounded, scrubbed OIDs, hashes, and changed paths.
...
- `vitest-related` collects the task's changed paths between the durable task-base tree and the current staged tree, excludes deletions, normalizes/deduplicates them, rebases them from the worktree root to `validationCwd`, then argv-spawns `npx vitest related --run --passWithNoTests <paths>` there. Rune and Rune-MCP opt into this strategy; deletion or global-config changes fall back to the complete command list.
- After commands pass, Rune stages and writes the tree again, re-captures against the same task base, and requires both the canonical hash and current-tree OID to equal the artifact approved by QA/reviewer/tech lead/designer. A post-review or validation-time mutation fails closed with durable `ReviewSurfaceFailure` base/candidate/current identities, hashes, and changed paths; raw diff content is never persisted.
