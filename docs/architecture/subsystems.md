...
**MCP metrics:** every registered MCP tool is wrapped in-process by `src/mcp/metrics.ts`. It tracks total calls, per-tool calls/errors/timeouts, bounded per-tool latency samples (fixed 1024-sample window), and reports active sessions plus warm-index status through the `mcp_metrics_snapshot` MCP tool. `RUNE_MCP_TOOL_TIMEOUT_MS` (default `30000`) is the per-call ceiling; exceeding it returns a tool-error result and counts as both error and timeout. Metrics are process-local and reset on daemon restart.

**Remote daemon tool contract:** the standalone daemon exposes the existing App-surface tools (`kb_query`, `vault_search`, `log_idea`, `crm_lookup`, `get_priorities`, `log_conversation`) plus five W1 tools: `journal_range`, `follow_wikilinks`, `tag_date_query`, `refresh_vault_index`, and `mcp_metrics_snapshot`. The local admin stdio server remains `ADMIN_TOOLS` only and does not expose these daemon utility/content tools. Tool results use the standard MCP text result shape (`content: [{type:"text", text:string}]`); validation, readiness, and handled operational failures resolve with `isError:true` instead of throwing. Production deps sanitize unexpected error text with secret redaction and absolute-path scrubbing before returning it to the caller.
