...
**Remote (`/mcp` Claude App connector):** `mountMcpRoute` (`mcp-transport.ts`) serves a Streamable HTTP MCP endpoint gated by single-user OAuth 2.1 (`mcp-oauth.ts`). Mounted only when `RUNE_HTTP_SECRET` is set. Gate order: host-allowlist 403 → FAIL-CLOSED bearer 401 → SDK transport. Per-session `McpServer` instances default to `createRuneMcpServer(APP_SURFACE_TOOLS)` (the six App-surface tools — kb_* admin tools are never remotely reachable). OAuth: DCR (http/https redirect_uris, MAX_CLIENTS=20), consent-form gate (POST with `RUNE_HTTP_SECRET` in the BODY), PKCE S256-only, codes single-use, tokens userId-bound + expiry-checked. Production wires `tokenTtlMs:null` (never-expire) + file store (`mcp-oauth-store.ts`, 0600, atomic) so the App authenticates ONCE. Serves RFC 8414 AS metadata + RFC 9728 protected-resource metadata (issuer pinned via `MCP_ISSUER_URL`). Revoke all = delete the store file + restart.
