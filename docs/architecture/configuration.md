...
- **`WORK_RUN_CLOSEOUT_COMMAND_TIMEOUT_MS`** — per-task closeout validation budget (ms), shared by the `vitest-related` and `product-commands` closeout strategies; a timeout enters the existing closeout repair/parking path (default `120000` = 2 min, min `1`). It does not affect coder self-validation or the final merge gate.
