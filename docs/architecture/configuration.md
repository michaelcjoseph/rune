...
- **`RUNE_INTERNAL_VALIDATION_COMPATIBLE_MODE`** — private, versioned marker set only by the sanitized validation launcher while confirming a structured `vitest related` host conflict. The compatible child remains under the normal outer validation Seatbelt; Rune-owned helpers check the exact marker before omitting their otherwise nested Seatbelt. Marker presence is not confinement proof by itself—the safety argument is the production launcher's provenance. It is not an operator/product setting, is absent from ordinary validation, and `assertOperatorConfig()` rejects any operator environment that defines it.
...
- **`policies/products.json`** — per-product sandbox config (repo path, base branch, credentials file, egress allowlist, `validationCommands`, optional `validationAdapters`, optional `validationCwd`, optional `orchestratedMode` boolean, optional `closeoutValidationStrategy`). Duplicate `validationCommands` fail config parsing. `validationAdapters` is a strict array of unique `{command, runner:"vitest"}` entries: each `command` must exactly equal one configured command; unknown runners, duplicate mappings, malformed arrays, and mappings for undeclared commands also fail parsing. Rune and Rune-MCP map their exact `npm test` command; an unmapped command still must exit zero but has unsupported—not canonical—coverage. `validationCwd` is repository-relative and defaults to the worktree root; orchestrated admission and the integration gate realpath it inside each concrete worktree, rejecting empty/absolute/parent-traversing, missing/non-directory, or symlink-escaping values. Closeout strategy accepts only `vitest-related` or `product-commands`; absent values default to `product-commands`, invalid values fail config parsing, and Rune/Rune-MCP opt into `vitest-related`. Assay sets `validationCwd: "harness/"` and runs, in order, `uv sync --all-groups`, `uv run pytest`, `uv run ruff check .`, and `uv run assay --help`. Exposed via `config.PRODUCTS_CONFIG_FILE`; read at runtime by `src/jobs/sandbox-runtime.ts`.
