...
- **`policies/products.json`** — per-product sandbox config (repo path, base branch, credentials file, egress allowlist, optional `orchestratedMode` boolean, optional `closeoutValidationStrategy`). Closeout strategy accepts only `vitest-related` or `product-commands`; absent values default to `product-commands`, invalid values fail config parsing, and Rune/Rune-MCP opt into `vitest-related`. Exposed via `config.PRODUCTS_CONFIG_FILE`; read at runtime by `src/jobs/sandbox-runtime.ts`.
