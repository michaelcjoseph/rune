...
- **`RUNE_PRODUCT_CHAT_FALLBACK_ROOT`** — root for unresolved/stale product-chat scratch workspaces; defaults to `~/.rune/product-chat-workspaces` and supports a leading `~/`. Rune creates one stable directory per product using the first 20 hex characters of its SHA-256 digest rather than the product slug. Before provisioning and again after realpath resolution, the root/product directory must be disjoint from `VAULT_DIR` and every configured product repository; overlap or an unexpected final symlink fails the chat closed. This setting is intentionally separate from broad `RUNE_WORKSPACE_DIR`.
...
- **`WORK_RUN_MAX_RUNTIME_MS`** — hard ceiling (ms) after which a run is group-killed and finalized regardless of apparent liveness (default `28800000` = 8 h, min `1`; project 15 P2.7). The only total-elapsed backstop — the liveness/output clocks are recency-based and can't catch an alive-and-active-but-endless run. Raised from 2 h so a legitimately long orchestrated run isn't cut mid-flight; the reconciler's presume-dead staleness stays at 2 h (decoupled, `work-run-reconciler.ts` `PRESUMED_DEAD_STALE_MS`).
- **`WORK_RUN_CLOSEOUT_COMMAND_TIMEOUT_MS`** — per-task closeout validation budget (ms), shared by the `vitest-related` and `product-commands` closeout strategies (default `120000` = 2 min, min `1`). An initial timeout enters the existing closeout repair/parking path; a timed-out compatible confirmation after a structured related-test host conflict instead becomes a WIP-preserving operational hold without consuming repair rounds. It does not affect coder self-validation or the final merge gate.
...
- **Canonical Vitest attestation handoff** — has no environment-variable contract. Rune embeds module-location-anchored observer/reporter/extractor source, private output path, and a fresh capability in source delivered through an anonymous stdin pipe; the exact configured command receives none of that material. The observer likewise sends extractor source through stdin to a capability-free subprocess whose explicit environment contains only toolchain `PATH` and `RUNE_VITEST_CACHE_DIR`. No attestation path, capability, implementation host path, or trust-root override is an operator/product setting or persisted receipt field.
- **`RUNE_VALIDATION_PROCESS_NONCE`** — fresh internal marker assigned by the shell-free validation launcher to each observer, exact command, related run, or compatible confirmation. Descendants normally inherit it; after process-group reap, asynchronous macOS `ps` scans use it to find, SIGKILL, and rescan detached/new-session descendants before evidence or closeout can proceed. This is cleanup correlation rather than an attestation credential and is not an operator/product setting.
- **`RUNE_VALIDATION_SANDBOX_BROKER_SOCKET`** — Unix socket of the Rune-owned broker enclosing a `sandbox-integration` shard, handed only to that broker's own children. Its presence lets a nested Rune-owned helper *ask* to reuse the already-applied outer Seatbelt; it never grants that by itself.
- **`RUNE_VALIDATION_CONFINEMENT_ATTESTATION`** — nonce minted per broker instance and handed to that broker's children. A child proves genuine enclosure by presenting it to the live broker over the socket above, which answers only for the exact nonce it issued and the profile it owns. A stale or forged pair authorizes nothing because the broker is gone or never issued that nonce. In-process grants use the launcher-created in-memory `ConfinementCapability` bound to the owner and exact socket instead.
- **`RUNE_INTERNAL_VALIDATION_COMPATIBLE_MODE`** — private transition marker retained only for the legacy, unprofiled `vitest related` compatible-confirmation path. It is neither a confinement credential nor accepted by Codex as authority to bypass an inner sandbox. It is not an operator/product setting and is absent from profiled validation.

`assertOperatorConfig()` rejects an operator environment that defines **any** of the four private launcher handoff variables above — none is an operator or product setting, so a stale `.env.local`, launchd plist, or shell-profile value would otherwise let a validation launch decision trust a value the launcher did not create.
...
- **`logs/work-runs/`** — root for per-work-run durable artifacts (project 11) — each run gets `<id>/` holding `transcript.jsonl`, `summary.json`, and (project 15) a `phase` file. Orchestrated task records may carry a bounded full-suite attestation plus compact closeout receipt; related-test closeout appends scrubbed `{recordedAt,taskId,...diagnostic}` rows to `related-test-diagnostics.jsonl`. A green independent merge gate atomically writes mode-0600 `gate-validation.json` before merge; its bounded receipt includes the canonical integration tree/review identity, completion time, command/configuration/dependency fingerprints, and per-command outcome/coverage counts. Malformed, contradictory, identity-free, missing, or red receipts never authorize merge, and historical runs may omit the file. Exposed via `config.WORK_RUNS_DIR`.
...
- **`policies/model-policy.json`** — declarative model registry + routing rules (aliases, providers, role-defaults, global-fallback); exposed via `config.MODEL_POLICY_FILE`. Carries fable + opus (anthropic/claude) and gpt-5.6-sol + gpt-5.6-terra (openai/codex). `DEFAULT_CHAT_MODEL` and `CONVERSATION_MODEL` select gpt-5.6-terra for new chat sessions; `src/ai/chat.ts` resolves that alias to its provider/format. Product-team roleDefaults remain separate: pm, tech-lead → fable; reviewer, designer → opus; qa → gpt-5.6-terra; coder → gpt-5.6-sol; reviewer/coder provider distinctness is enforced fail-closed at resolution time in `team-task-deps.ts`. `planning-critique` (the `/plan` cross-model pass) → gpt-5.6-sol, resolved with `distinctFromProvider: 'anthropic'` so it can never route a Claude model into `runCodex`. `src/index.ts` loads and validates the policy at startup, failing fast on a malformed file. A missing file remains tolerated for agent execution (`runAgent()` falls back to `def.model ?? config.AGENT_MODEL`), but chat requires its selected alias to resolve when `MODEL_POLICY_FILE` is configured.
...
- **`policies/products.json`** — per-product sandbox config (repo path, base branch, credentials file, egress allowlist, `validationCommands`, required `validationCommandProfiles`, optional `validationAdapters`, optional `validationCwd`, optional `orchestratedMode` boolean, optional `closeoutValidationStrategy`). Duplicate `validationCommands` fail config parsing. `validationCommandProfiles` must assign every exact command exactly once to `isolated`, `loopback`, or `sandbox-integration`; missing, duplicate, unknown-command, and unknown-profile assignments reject the complete live config. `validationAdapters` is a strict array of unique `{command, runner:"vitest", profileSelection?:"strict-tags-v1"}` entries: each command must exactly equal one configured command, and malformed selectors/runners or duplicate/undeclared mappings fail parsing. Strict selection expands that command in stable `isolated` → `loopback` → `sandbox-integration` order using Vitest's `validation-loopback` and `validation-sandbox-integration` tags; untagged tests select `isolated`, and a test carrying both capability tags is invalid. Rune and Rune-MCP enable strict selection for exact `npm test`; other current commands start at their declared minimum profile. An unmapped command still must exit zero but has unsupported—not canonical—coverage. `validationCwd` is repository-relative and defaults to the worktree root; orchestrated admission and the integration gate realpath it inside each concrete worktree, rejecting empty/absolute/parent-traversing, missing/non-directory, or symlink-escaping values. Closeout strategy accepts only `vitest-related` or `product-commands`; absent values default to `product-commands`, invalid values fail config parsing, and Rune/Rune-MCP opt into `vitest-related`. Assay sets `validationCwd: "harness/"` and runs, in order, `uv sync --all-groups`, `uv run pytest`, `uv run ruff check .`, and `uv run assay --help`, all under `isolated`. `uv sync` is deliberately NOT `loopback`: every profile denies external network by mandate, so no profile can grant package downloads, and `uv sync` needs no localhost — `isolated` is its true minimum. Making Assay's Python dependencies resolvable is a provisioning problem, not a profile problem, and is tracked by the Python dependency-provisioning entry in `docs/projects/bugs.md`. Exposed via `config.PRODUCTS_CONFIG_FILE`; read at runtime by `src/jobs/sandbox-runtime.ts`.
