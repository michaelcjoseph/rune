...
- **`logs/work-runs/`** — root for per-work-run durable artifacts (project 11) — each run gets `<id>/` holding `transcript.jsonl`, `summary.json`, and (project 15) a `phase` file; orchestrated related-test closeout additionally appends scrubbed `{recordedAt,taskId,...diagnostic}` rows to `related-test-diagnostics.jsonl`. Exposed via `config.WORK_RUNS_DIR`.
