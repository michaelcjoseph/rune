...
- **`RUNE_INTERNAL_VALIDATION_COMPATIBLE_MODE`** — private transition marker retained only for the legacy, unprofiled `vitest related` compatible-confirmation path. It is neither a confinement credential nor accepted by Codex as authority to bypass an inner sandbox; inherited confinement now requires an in-memory launcher-created capability bound to the exact owner and profile path. It is not an operator/product setting, is absent from profiled validation, and `assertOperatorConfig()` rejects any operator environment that defines it.
