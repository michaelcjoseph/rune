...
│   ├── work-run-commit-poll.ts # Phase 4 scaffold (pure commit-poll throttle core, not yet wired): `planCommitProgress(state, commits, tally, now, throttleMs)` — parent-side throttle decision for the per-run commit-progress alert; state carries `{lastSeenSha, lastPingAt}` (new sha seen + throttle gap → `{ping, message?, nextState}`; no new sha or within throttle window → `{ping:false}`); body notImplemented; runtime glue (setInterval + git rev-list + bus publish) lands in work-runner in the Phase 4 impl task
...
│   ├── stall-check.ts       # Pure stall-check core: checkStalledRuns(deps) returns newly-nudged-id set; formatStallNudge(run, now) builds the Telegram nudge string; exports TICK_INTERVAL_MS (30s) and STALL_THRESHOLD_MS (5min); Phase 4 scaffold: QUIET_THRESHOLD_MS (5min, measured on lastOutputAt distinct from child liveness) + formatQuietNudge(run, now) (body notImplemented — wording distinguishes quiet-but-alive from child-dead stall)
...
│   ├── supervision.ts       # Supervision (Layer 3): visibility surface over long-running runs — isStalled, getVisibility (active/blocked/stalled), markCrashed (crash → terminal), recoverRun (restart → unknown), recordHeartbeat; pure over (runs, heartbeatIntervalMs, now); Phase 4 scaffold: two new optional SupervisedRun fields (`lastOutputAt?` — ISO-8601 of most recent Claude output event, distinct from lastHeartbeatAt/lastChildAliveAt; `quietNudgedAt?` — ISO-8601 of once-only quiet nudge send); `isQuietRun(run, quietThresholdMs, now)` and `planQuietNudges(runs, quietThresholdMs, now)` → `QuietNudgePlan` (bodies notImplemented)
