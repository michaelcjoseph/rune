...
- **Validation isolation:** product-controlled validation receives only basic toolchain environment plus its isolated cache—never Rune/integration secrets—and on macOS runs with external networking denied (localhost remains available for test servers). A `vitest related` host conflict requires complete bounded structured evidence; Rune reruns the exact selection under the same outer confinement, and only launcher-marked Rune helpers may omit a nested Seatbelt (the marker is not itself proof of confinement and operator startup rejects it). The confirmation must pass; otherwise the run becomes an operational hold without spending coder/QA repair rounds. Timeout handling must reap the whole process group through SIGKILL before returning (`src/jobs/work-run-gate-runtime.ts`, `src/jobs/related-test-diagnostic.ts`).
