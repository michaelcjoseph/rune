...
│   ├── work-run-projection.ts # readWorkRunProjections(dir, indexFile, recent?): reads logs/work-runs/ index.jsonl + per-run summary.json (via `readWorkRunSummary`) + transcript.jsonl tail into a slug-keyed WorkRunProjection map; server-layer bridge between the jobs-layer store and the intent-layer cockpit view; best-effort (per-run failures skipped); holds the compile-time drift guard between jobs WorkOutcome and intent WorkRunOutcome; `transcriptUrl` points at `GET /api/work-runs/:id/transcript` when a transcript file exists, null otherwise
