...
- **Chat scoping** (`src/vault/sessions.ts` `buildSessionSystemPrompt`) — a **global** chat (Telegram / cockpit Home) is the read-only vault thinking-partner. A **product** chat (cockpit per-product chat) is a write-enabled development agent for that product: it spawns with cwd = the product repo and `PRODUCT_CHAT_TOOLS` (Edit/Write/Bash). Edit/Write are hard-confined to the repo via `writableRoots` (the blanket `WORKSPACE_DIR` add-dir is dropped); Bash runs builds/tests but is unbounded, so its vault-write boundary is a prompt instruction + the vault's git recoverability, not OS-level. The vault is read-only via the `rune-kb` MCP.
