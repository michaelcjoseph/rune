...
│   ├── work-run-transcript.ts # Project 11 Phase 1 module surface: stream-json→display adapter (parseStreamJsonLine, streamJsonToDisplay), per-run transcript WriteStream with backpressure + awaited finish (createTranscriptSink), last-N ring buffer + stderr tail (createRingBuffer), secret redaction (redactSecrets); exported types + signatures scaffolded, implementation pending
