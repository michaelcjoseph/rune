...
**Product-team roles (project 14, `agents/<role>/` SOUL + memory):** pm, tech-lead, qa, coder, reviewer, designer, security, adjudicator (tie-breaks a reviewer-vs-tech-lead split that would otherwise end a task). Designer and security are conditional closeout gates selected by explicit planning flags. Plus the writer role (`agents/writer/`). → `docs/architecture/subsystems.md` and `reviews-kb-vault.md`.
