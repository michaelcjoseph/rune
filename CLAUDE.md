...
│   └── backlog-reader.ts    # Filesystem + security layer over backlog-parser: readBacklogs(registry, productsConfig, opts?) → ProductBacklog[]; realpaths each repoPath + requires it under $WORKSPACE_ROOT, rejects symlink escapes, fd-based size-capped reads, repo-relative source paths; minimal BacklogReaderConfig avoids an intent→jobs ProductConfig dependency
├── mcp/
