...
- **Chat scoping** (`src/vault/sessions.ts` `buildSessionSystemPrompt`) — a **global** chat (Telegram / cockpit Home) is the read-only vault thinking-partner. A **product** chat (cockpit per-product chat) is a write-enabled development agent for that product: it spawns with repo-root cwd and `PRODUCT_CHAT_TOOLS` (Edit/Write/Bash). Edit/Write are confined to the product workspace via `writableRoots` (for scoped products, `repoPath/scopePath`; the blanket `WORKSPACE_DIR` add-dir is dropped). Bash runs builds/tests from repo root, with Rune secrets scrubbed from env; it remains unbounded, so its vault-write boundary is prompt instruction + git recoverability, not OS-level. The vault is read-only by policy via the `rune-kb` MCP.
