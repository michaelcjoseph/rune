...
- **Validation isolation and attestation:** product-controlled validation receives only basic toolchain environment plus its isolated cache—never Rune/integration secrets—and on macOS runs with external networking denied (localhost remains available for test servers). `validationAdapters` may map exact configured commands to the trusted `vitest` observer; only an authenticated, reconciled Rune-owned manifest bound to the unchanged canonical tree/review hash, complete configured argv, and validation fingerprints establishes reusable full-suite coverage. Rune resolves and boot-anchors reporter/observer/extractor source plus the installed Vitest/@vitest/Vite runtime from the Rune module location, never a configurable product root. For an eligible command it first observes a post-realpath-contained clean materialization of the exact reviewed tree with Rune-owned dependencies, delivering source, output path, and one-run capability only through anonymous stdin; product config is evaluated by a separate capability-free extractor and tests run in forced fork workers. On macOS the observer's Seatbelt denies file writes to Rune's trust root and the reviewed materialization. The independently executed exact command denies the Rune trust root but explicitly re-allows its concrete lexical/real validation worktree, including worktrees nested under Rune's default `.worktrees` root. Observer setup/rejection never suppresses that exact configured command. Unsupported commands still must pass but never claim coverage. A `vitest related` host conflict requires complete bounded structured evidence; Rune reruns the exact selection under the same outer confinement, and only launcher-marked Rune helpers may omit a nested Seatbelt (the marker is not itself proof of confinement and operator startup rejects it). Every validation exit reaps its process group and asynchronously scans/reaps nonce-marked detached descendants on macOS before evidence or closeout mutation (`src/jobs/work-run-gate-runtime.ts`, `src/jobs/full-suite-attestation.ts`, `src/jobs/related-test-diagnostic.ts`).
