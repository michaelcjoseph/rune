...
│   ├── work-run-transcript.ts # Stream-json→display adapter: `parseStreamJsonLine` (tolerant parse) + `streamJsonToDisplay` (converts envelopes to display text, reuses `formatToolUse`/`scrubPathsInText` from `src/ai/tool-labels.ts`) — IMPLEMENTED; `redactSecrets`, `createRingBuffer`, `createTranscriptSink` remain scaffolds for the next "Durable sink" task
