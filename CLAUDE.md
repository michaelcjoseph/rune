...
- Chat-turn locks serialize the complete session transaction per transport/user/scope while unrelated chats remain concurrent; provider session locks separately prevent concurrent CLI writes to one session ID. Git commits happen at key moments (morning prep, `/fresh`, nightly), not on timers.
