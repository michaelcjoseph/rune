...
- **HTTP server** (localhost:3847) — health endpoint, session capture for nightly; webview UI (Phase B) serves a vanilla HTML/JS chat interface at `/` with cookie auth, REST endpoints (`GET /api/cockpit`, `GET /api/backlog/:product`, `POST /api/mutations`, `POST /api/mutations/:id/cancel`, `POST /api/ops/:id/cancel`, `POST /api/server/restart`, `GET /api/work-runs/:id`, `GET /api/work-runs/:id/transcript`), and a WebSocket for real-time messaging; the cockpit sidebar panel polls `GET /api/cockpit` and renders each product's projects with lifecycle status, run-status, and per-project action buttons (start / continue / enter-planning-mode). A production-only "↻ Restart server" button at the top of the sidebar posts to `POST /api/server/restart` (`src/server/restart.ts` → detached `launchctl kickstart -k gui/<uid>/$LAUNCHD_LABEL`; the plist's KeepAlive only respawns on crash, so a clean exit won't restart — kickstart is the explicit relaunch path); the endpoint 409s and the button stays hidden unless the server injects `is-production=true` into the index.html template
