...
- **HTTP server** (localhost:3847) — health endpoint, session capture, Whoop OAuth, and the **webview/cockpit** at `/` (cookie auth + host-guard): a vanilla HTML/JS chat UI plus a cockpit sidebar (per-product/project lifecycle + run status + action buttons), backlog drawer, planning panel, pending-approvals, product deep-view, and a real-time run-feed. REST + WebSocket; product-chat turns carry stable turn/message IDs so unacknowledged commit receipts can replay after reconnect.
- **Chat scoping** (`src/vault/sessions.ts` `buildSessionSystemPrompt`) — a **global** chat (Telegram / cockpit Home) is the read-only vault thinking-partner. A **resolved product** chat (cockpit per-product chat, webview-only — TG never gets product scope) is a write-enabled development agent for that product: a canonical repository cwd/scoped root + `PRODUCT_CHAT_TOOLS` (Edit/Write/Bash) + the dedicated strict seven-tool `rune-kb`/Cockpit MCP surface. The optional scope must be relative, exist, and realpath inside the repository; validation occurs before repo context is read, and the validated context is reused for the turn. A missing/stale/malformed/symlink-escaping repo or scope becomes an **unresolved product** chat in a dedicated per-product hashed scratch directory under `PRODUCT_CHAT_FALLBACK_ROOT` (default `~/.rune/product-chat-workspaces`), which is realpath-checked to stay disjoint from the vault and every configured product repo. Fallback gets base read/edit/shell tools but no KB/Cockpit tools: Claude receives a strict empty MCP config; Codex uses strict config parsing, receives an empty MCP table, ignores user config and exec-policy rules, disables hooks/apps/remote plugins and network access, and removes temp/extra writable roots. Codex maps global/fallback/resolved authority to `read-only`/`workspace-write`/`danger-full-access` and reasserts it on resumed turns. **Containment is soft for Claude product chat, not OS-enforced:** the spawn uses `--dangerously-skip-permissions`, under which `--add-dir`/`writableRoots` do NOT restrict writes and Bash has full filesystem access — so the narrowed writable-root hints and `product-chat` env scrub are *defense-in-depth*, not boundaries. Vault-write and Rune-secret protection rest on the system prompt + the vault's git recoverability; resolved chats reach the vault read-only via `rune-kb`, while fallback chats receive no vault tool. **This is a deliberate, accepted posture** (single-user, localhost, cookie-authed surface — same access level as the operator running Claude Code): Claude product chat is intentionally a full-trust local agent, NOT a sandboxed one. A hard OS guarantee would need `sandbox-exec` (deny vault writes + secret-file reads); intentionally not wired. Don't "fix" the soft boundary as if it were an oversight — re-decide with the operator first.
...
- **Work-run lifecycle** — work runs stream a durable transcript, classify on work product, and finalize through a gated-merge finalizer (project 11/13/15). Post-coder review is a durable concurrent quorum: one non-blocking reviewer/tech-lead/conditional-security verdict clears the eligible quorum, any completed objection wins, and a conditional designer remains a separate required gate. Required orchestrated tasks then run Rune-owned configured full validation; typed adapters can attest exact reviewed-tree coverage for closeout reuse, while the independent integration-worktree merge gate always reruns every configured command. Supervision + stall-check monitor liveness; parked runs wait for a human release. → `docs/architecture/subsystems.md`; full `/plan`→merged stage-by-stage map (agents/gates/criteria) → `docs/architecture/project-lifecycle.md`.
...
- **`ai/`** — provider-aware chat routing + all Claude/Codex CLI spawning (`claude.ts` is the only Claude spawn chokepoint) + resolved product-chat commit verification + tool labels.
...
- **`server/`** — HTTP, webview/cockpit + REST/WS API (including durable product-chat receipt delivery), MCP transport + OAuth, auth, snapshots.
...
- **`writer/`, `roles/`** — role-agent SOUL + memory loaders (writer + the eight product-team roles).
...
- All timestamps use `America/Chicago` (`src/utils/time.ts`). Config reads from env vars; defaults in `src/config.ts`. Telegram operator credentials are lazy getters so scoped MCP subprocesses can import shared modules without fake credentials; the main entrypoint must call `assertOperatorConfig()` before boot side effects to retain fail-fast operator startup.
...
- **Provider termination is classified by cause:** `SIGTERM`/exit 143 is a timeout only when Rune's own timer fired; otherwise Claude/Codex preserve cancellation or typed `executor-exit` evidence (including a bounded CLI terminal reason) instead of inventing a timeout.
...
- Chat-turn locks serialize the complete session transaction per transport/user/scope while unrelated chats remain concurrent; provider session locks separately prevent concurrent CLI writes to one session ID. Git commits happen at key moments (morning prep, `/fresh`, nightly, or an explicit resolved product-chat request), not on timers.
- **Resolved product-chat commits are verified and durable:** only a direct commit request in `product-full-access` may invoke the bounded, credential-stripped Git verifier. Capture baseline `HEAD` before provider execution; report success only when the provider succeeds, the new `HEAD` is a verified descendant with valid metadata, and the worktree is clean. Commit intent is a clause-level imperative test, never the mere presence of the word — prose *about* commits must not acquire verification authority. With a client `turnId`, flush a **commit** turn's pending intent to disk before execution (an ordinary turn defers that write to completion) and the terminal outcome/message/executor before delivery; a turn that cannot be recorded still terminates with a reply rather than propagating out of the WS dispatch chain. A `turnId` replayed with different request text is a conflict, not a replay. Retries replay the stored terminal message, while interrupted pending commit turns become unconfirmed and are never re-executed (`src/ai/product-chat-commit.ts`, `src/vault/sessions.ts`).
...
- **Validation isolation and attestation:** every exact `validationCommands` entry has one fail-closed `validationCommandProfiles` assignment (`isolated`, `loopback`, or `sandbox-integration`). Vitest adapters may opt into `strict-tags-v1`, which deterministically shards untagged, `validation-loopback`, and `validation-sandbox-integration` tests in that order; conflicting capability tags are rejected. Rune probes every selected profile before dispatch. `isolated` denies all networking, `loopback` permits only localhost, and sandbox assertions use a narrow Rune-owned Unix-socket broker that executes fixed top-level Seatbelt probes—never arbitrary product commands. Product validation still receives only basic toolchain environment plus its isolated cache, never Rune/integration secrets. Profile plans/outcomes and their definition fingerprints are part of V2 receipts/attestations, so V1 evidence stays readable but cannot authorize profiled reuse. A missing capability yields typed `profile-unavailable` operational hold, never coder feedback. Conflicting-tag rejection is enforced in the trusted Vitest reporter (the only component that sees per-test resolved tags), which counts each conflicting test as a collection error so the shard fails closed instead of silently running it in no profile. Codex's external-sandbox bypass and an in-process broker grant require a launcher-created in-memory capability bound to the owner and exact profile; a child's cross-process bypass requires the live enclosing broker to confirm the nonce it minted for the profile it owns. Neither the legacy environment marker nor the presence of any private handoff variable is proof — `assertOperatorConfig()` rejects an operator environment defining any of them. Preserve boot-anchored trusted Vitest source/runtime, reviewed-tree identity, worktree/trust-root containment, process-group + detached-descendant reaping, and independent merge-gate reruns (`src/intent/validation-profiles.ts`, `src/jobs/work-run-gate-runtime.ts`, `src/jobs/validation-sandbox-broker.ts`, `src/jobs/full-suite-attestation.ts`).
- **Artifact-role MCP:** `artifactMcp: "rune-kb-readonly"` is macOS/Seatbelt-only and applies only to explicit QA/coder executions. A supervised privileged broker receives the validated vault path over stdin and serves exactly `vault_search`, `journal_range`, and `follow_wikilinks` over a temporary Unix socket; the model-facing relay receives no vault path or Rune secrets. Claude/Codex artifact children run under a generated Seatbelt profile denying direct vault reads/writes; Codex also ignores user config, replaces the complete MCP table, requires the server, pins the tool allowlist, and approves those three unattended reads. Never load `.env.local`, inherit global MCP registrations, expose mutation/admin tools, or weaken symlink containment, fail-closed setup, or process-group/socket cleanup (`src/jobs/artifact-mcp.ts`, `src/mcp/artifact-readonly*.ts`).
- **Pre-coder invariant review is flag-contained, not OS-contained:** the `securityNeeded` tech-lead-draft and security-ratification calls (`defaultInvariantReviewCall`, `src/jobs/team-task-deps.ts`) are read-only by way of `availableTools: ['Read','Glob','Grep']` → the CLI's `--tools` flag, plus a fail-closed non-Claude rejection, a strict empty MCP config, `product-chat` env scrub, and cwd/`writableRoots` pinned to the run's own worktree. `execClaude` prepends `--dangerously-skip-permissions` unconditionally, so that tool narrowing IS the whole read-only boundary — and it rests on Claude CLI behavior this repo cannot assert (tests cover argv construction only; the adjacent `--agents` JSON `tools: []` claim at `src/ai/claude.ts` is the same kind of unverified precedent). **This is a deliberate, accepted posture (decided 2026-08-07), not an oversight.** A hard guarantee would mean routing these two calls through the Seatbelt-sandboxed `rune-kb-readonly` broker pattern; intentionally not wired, because every other role call in the same run (QA, coder) already spawns with Bash under identical skip-permissions — the narrowing reduces surface, it does not establish a boundary Rune depends on. Preserve the fail-closed non-Claude rejection, the empty MCP config, and the worktree pinning; treat the `--tools` narrowing as defense-in-depth. Don't promote this to a sandbox as if it were a bug — re-decide with the operator first, same as the product-chat soft boundary above.
...
- **Orchestrated terminal truth:** keep the immutable success/failure/cancellation trigger separate from removed/preserved/parked disposition; resolve disposition and append terminal transcript facts before final terminal mutation/supervision publication and the final summary rewrite. Execution failures use `execution-failure`; non-user cancellations use `system-cancel` with no invented process exit code. → `docs/architecture/subsystems.md`.
...
npm run diagnose:reviewed-tree       # name the tests that fail only in the observer's materialized reviewed tree
```
...
| Variable | Purpose |
|---|---|
| `TELEGRAM_BOT_TOKEN`, `TELEGRAM_USER_ID`, `VAULT_DIR` | **Required for the main operator process** — bot token, your numeric ID, vault path; scoped MCP children omit unrelated Telegram credentials |
...
| `RUNE_PRODUCT_CHAT_FALLBACK_ROOT` | Dedicated unresolved-product scratch root (default `~/.rune/product-chat-workspaces`) |
...
**Runtime (generic tooling, this repo):** wiki-compiler, kb-query, wiki-linter, morning-prep, session-summarizer, release-notes, content-triager, note-triage, observation-diarizer, observation-triage, photo-classifier, system-scanner, project-updater, playbook-proposer, playbook-updater, proposal-updater, worldview-updater, psychology-updater, json-updater, daily-content-updater, intent-scan, workout-generator, lenny-sync, sr-question-generator, sr-grader, project-setup-writer.
...
**Product-team roles (project 14, `agents/<role>/` SOUL + memory):** pm, tech-lead, qa, coder, reviewer, designer, security, adjudicator (tie-breaks a reviewer-vs-tech-lead split that would otherwise end a task). Designer is a conditional required closeout gate; conditional security participates in the eligible review quorum and can still object. Plus the writer role (`agents/writer/`). → `docs/architecture/subsystems.md` and `reviews-kb-vault.md`.
