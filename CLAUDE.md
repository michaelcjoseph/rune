...
- **Artifact-role MCP:** `artifactMcp: "rune-kb-readonly"` is the only accepted product policy and applies only to explicit QA/coder executions. A privileged read-only broker receives the validated vault path over stdin and serves exactly `vault_search`, `journal_range`, and `follow_wikilinks` over a temporary Unix socket; the model-facing stdio relay receives no vault path or Rune secrets. Claude/Codex artifact children run under a generated Seatbelt profile that denies direct vault reads/writes; Codex also ignores user config, replaces the complete MCP table, requires the server, pins the tool allowlist, and approves those three unattended reads. Never load `.env.local`, inherit global MCP registrations, expose mutation/admin tools, or weaken symlink containment/fail-closed setup (`src/jobs/artifact-mcp.ts`, `src/mcp/artifact-readonly*.ts`).
