...
│   └── sandbox.ts           # Sandboxing and security (Layer 4): sandbox boundary-policy core — worktreePathFor (per-project worktree path, slug-validated), isWriteAllowed (lexical containment, path-traversal safe), isEgressAllowed (exact-match allowlist), canReachCredential (product-scoped)
├── mcp/
